...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CCICcoinAddress(redeemScript.GetID()).ToString();
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        // CICs of the unencrypted private key in slack space in the database file.
...
CWallet::TxItems CWallet::OrderedTxItems(std::list<CAccountingEntry>& acentries, std::string strAccount)
{
    ...
    acentries.clear();
    walletdb.ListAccountCreditDeCIC(strAccount, acentries);
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFromLoadWallet)
{
    ...
    else
    {
        ...
        // Break deCIC/credit balance caches:
        wtx.MarkDirty();
...
int64_t CWallet::GetDeCIC(const CTxIn &txin) const
{
    {
        LOCK(cs_wallet);
        map<uint256, CWalletTx>::const_iterator mi = mapWallet.find(txin.prevout.hash);
                           ...
                           list<pair<CTxDestination, int64_t> >& listSent, int64_t& nFee, string& strSentAccount) const
{
    ...
    // Compute fee:
    int64_t nDeCIC = GetDeCIC();
    if (nDeCIC > 0) // deCIC>0 means we signed/sent this transaction
    {
        int64_t nValueOut = GetValueOut();
        nFee = nDeCIC - nValueOut;
    }
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // If we are deCICed by the transaction, add the output as a "sent" entry
        if (nDeCIC > 0)
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
                {
                    ...
                    //reflecting an assumption the user would accept a CIC more delay for
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-CICcoin-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CCICcoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CCICcoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CCICcoinAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CCICcoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CCICcoinAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CCICcoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CCICcoinAddress(dest).ToString(), key);
}
