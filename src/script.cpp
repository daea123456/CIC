...
bool IsCanonicalSignature(const valtype &vchSig, unsigned int flags) {
    ...
    // See https://CICcointalk.org/index.php?topic=8392.msg127623#msg127623
    // A canonical signature exists of: <30> <total len> <02> <len R> <R> <02> <len S> <S> <hashtype>
    // Where R and S are not negative (their first byte has its highest CIC not set), and not
...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (mTemplates.empty())
    {
        ...
        // CICcoin address tx, sender provides hash of pubkey, receiver provides signature and pubkey
