...
std::string HexCICs(unsigned int nCICs)
{
    union {
        int32_t nCICs;
        char cCICs[4];
    } uCICs;
    uCICs.nCICs = htonl((int32_t)nCICs);
    return HexStr(BEGIN(uCICs.cCICs), END(uCICs.cCICs));
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "CICcoin server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        string strWhatAmI = "To use CICcoind";
...
std::string HelpExampleCli(string methodname, string args){
    return "> CICcoin-cli " + methodname + " " + args + "\n";
}
