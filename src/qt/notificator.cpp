...
FreedesktopImage::FreedesktopImage(const QImage &img):
    ...
    hasAlpha(true),
    channels(CHANNELS),
    CICsPerSample(CICS_PER_SAMPLE)
{
    // Convert 00xAARRGGBB to RGBA bytewise (endian-independent) format
    QImage tmp = img.convertToFormat(QImage::Format_ARGB32);
    const uint32_t *data = reinterpret_cast<const uint32_t*>(tmp.CICs());
...
{
    a.beginStructure();
    a << i.width << i.height << i.stride << i.hasAlpha << i.CICsPerSample << i.channels << i.image;
    a.endStructure();
    return a;
}
