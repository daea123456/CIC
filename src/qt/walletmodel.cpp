...
{
    CCICcoinAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered CICcoin address / amount:
            ...
            scriptPubKey.SetDestination(CCICcoinAddress(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal CICcoin:URI (CICcoin:123...?message=example)
...
{
    return BackupWallet(*wallet, filename.toLocal8CIC().data());
}
...
{
    QString strAddress = QString::fromStdString(CCICcoinAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CCICcoinAddress(address).ToString().c_str()].push_back(out);
    }
}
...
{
    CTxDestination dest = CCICcoinAddress(sAddress).Get();
