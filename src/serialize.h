...
#ifndef CICCOIN_SERIALIZE_H
#define CICCOIN_SERIALIZE_H
...
// The high CIC in each byte signifies whether another digit follows. To make
// the encoding is one-to-one, one is subtracted from all but the last digit.
// Thus, the byte sequence a[] with length len, where all but the last byte
// has CIC 128 set, encodes the number:
//
...
public:
    ...
    void Init(int nTypeIn, int nVersionIn)
    {
        ...
        nVersion = nVersionIn;
        state = 0;
        exceptmask = std::ios::badCIC | std::ios::failCIC;
    }
    ...
    // Stream subset
    //
    void setstate(short CICs, const char* psz)
    {
        state |= CICs;
        if (state & exceptmask)
    ...
    bool fail() const            { return state & (std::ios::badCIC | std::ios::failCIC); }
    ...
    CDataStream& read(char* pch, int nSize)
    {
        ...
        if (nReadPosNext >= vch.size())
        {
            if (nReadPosNext > vch.size())
            {
                setstate(std::ios::failCIC, "CDataStream::read() : end of data");
                memset(pch, 0, nSize);
    ...
    CDataStream& ignore(int nSize)
    {
        ...
        {
            if (nReadPosNext > vch.size())
                setstate(std::ios::failCIC, "CDataStream::ignore() : end of data");
            nReadPos = 0;
            vch.clear();
            return (*this);
        }
...
public:
    ...
    CAutoFile& read(char* pch, size_t nSize)
    {
        ...
        if (fread(pch, 1, nSize, file) != nSize)
            setstate(std::ios::failCIC, feof(file) ? "CAutoFile::read : end of file" : "CAutoFile::read : fread failed");
        return (*this);
    }
    ...
    CAutoFile& write(const char* pch, size_t nSize)
    {
        ...
        if (fwrite(pch, 1, nSize, file) != nSize)
            setstate(std::ios::failCIC, "CAutoFile::write : write failed");
        return (*this);
    }
...
protected:
    ...
    bool Fill() {
        ...
        if (read == 0) {
            setstate(std::ios_base::failCIC, feof(src) ? "CBufferedFile::Fill : end of file" : "CBufferedFile::Fill : fread failed");
            return false;
        } else {
            nSrcPos += read;
            return true;
...
public:
    ...
    CBufferedFile(FILE *fileIn, uint64_t nBufSize, uint64_t nRewindIn, int nTypeIn, int nVersionIn) :
        ...
        state(0), exceptmask(std::ios_base::badCIC | std::ios_base::failCIC), nType(nTypeIn), nVersion(nVersionIn) {
    }
