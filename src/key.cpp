...
public:
    ...
    bool SignCompact(const uint256 &hash, unsigned char *p64, int &rec) {
        ...
        if (sig==NULL)
            return false;
        memset(p64, 0, 64);
        int nCICsR = BN_num_CICs(sig->r);
        int nCICsS = BN_num_CICs(sig->s);
        if (nCICsR <= 256 && nCICsS <= 256) {
            CPubKey pubkey;
            ...
            assert(fOk);
            BN_bn2bin(sig->r,&p64[32-(nCICsR+7)/8]);
            BN_bn2bin(sig->s,&p64[64-(nCICsS+7)/8]);
        }
        ECDSA_SIG_free(sig);
        return fOk;
    }
    ...
    static bool TweakSecret(unsigned char vchSecretOut[32], const unsigned char vchSecretIn[32], const unsigned char vchTweak[32])
    {
        ...
        int nCICs = BN_num_CICs(bnSecret);
        memset(vchSecretOut, 0, 32);
        BN_bn2bin(bnSecret, &vchSecretOut[32-(nCICs+7)/8]);
        EC_GROUP_free(group);
        BN_CTX_end(ctx);
        BN_CTX_free(ctx);
        return ret;
