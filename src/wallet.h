...
#ifndef CICCOIN_WALLET_H
#define CICCOIN_WALLET_H
...
public:
    ...
    int64_t GetDeCIC(const CTxIn& txin) const;
    ...
    {
        return (GetDeCIC(tx) > 0);
    }
    int64_t GetDeCIC(const CTransaction& tx) const
    {
        int64_t nDeCIC = 0;
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            nDeCIC += GetDeCIC(txin);
            if (!MoneyRange(nDeCIC))
                throw std::runtime_error("CWallet::GetDeCIC() : value out of range");
        }
        return nDeCIC;
    }
...
public:
    ...
    // memory only
    mutable bool fDeCICCached;
    mutable bool fCreditCached;
    mutable bool fImmatureCreditCached;
    mutable bool fAvailableCreditCached;
    mutable bool fChangeCached;
    mutable int64_t nDeCICCached;
    ...
    int64_t GetDeCIC() const
    {
        if (vin.empty())
            return 0;
        if (fDeCICCached)
            return nDeCICCached;
        nDeCICCached = pwallet->GetDeCIC(*this);
        fDeCICCached = true;
        return nDeCICCached;
    }
    ...
    bool IsTrusted() const
    {
        ...
        if (nDepth < 0)
            return false;
        if (!bSpendZeroConfChange || !IsFromMe()) // using wtx's cached deCIC
            return false;
