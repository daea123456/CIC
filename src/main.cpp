...
#if defined(NDEBUG)
# error "CICcoin cannot be compiled without assertions."
#endif
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block.GetHash(), block.nCICs))
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if ((pindexLast->nHeight+1) % nInterval != 0)
    {
        if (TestNet())
        {
            ...
            else
            {
                ...
                while (pindex->pprev && pindex->nHeight % nInterval != 0 && pindex->nCICs == nProofOfWorkLimit)
                    pindex = pindex->pprev;
                return pindex->nCICs;
            }
        }
        return pindexLast->nCICs;
    }
    ...
    // Retarget
    CBigNum bnNew;
    bnNew.SetCompact(pindexLast->nCICs);
    ...
    LogPrintf("Before: %08x  %s\n", pindexLast->nCICs, CBigNum().SetCompact(pindexLast->nCICs).getuint256().ToString());
    LogPrintf("After:  %08x  %s\n", bnNew.GetCompact(), bnNew.getuint256().ToString());

    return bnNew.GetCompact();
}

bool CheckProofOfWork(uint256 hash, unsigned int nCICs)
{
    CBigNum bnTarget;
    bnTarget.SetCompact(nCICs);

    // Check range
    if (bnTarget <= 0 || bnTarget > Params().ProofOfWorkLimit())
        return error("CheckProofOfWork() : nCICs below minimum work");

    // Check proof of work matches claimed amount
    if (hash > bnTarget.getuint256())
        return error("CheckProofOfWork() : hash doesn't match nCICs");
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    // We use 7 blocks rather arCICrarily as it represents just under 10% of sustained network
...
void UpdateTime(CBlockHeader& block, const CBlockIndex* pindexPrev)
{
    ...
    if (TestNet())
        block.nCICs = GetNextWorkRequired(pindexPrev, &block);
}
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Check proof of work matches claimed amount
    if (fCheckPOW && !CheckProofOfWork(block.GetHash(), block.nCICs))
...
bool AcceptBlock(CBlock& block, CValidationState& state, CDiskBlockPos* dbp)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        // Check proof of work
        if (block.nCICs != GetNextWorkRequired(pindexPrev, &block))
            return state.DoS(100, error("AcceptBlock() : incorrect proof of work"),
                             REJECT_INVALID, "bad-diffCICs");
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if (pcheckpoint && pblock->hashPrevBlock != (chainActive.Tip() ? chainActive.Tip()->GetBlockHash() : uint256(0)))
    {
        ...
        CBigNum bnNewBlock;
        bnNewBlock.SetCompact(pblock->nCICs);
        CBigNum bnRequired;
        bnRequired.SetCompact(ComputeMinWork(pcheckpoint->nCICs, deltaTime));
...
void CPartialMerkleTree::TraverseAndBuild(int height, unsigned int pos, const std::vector<uint256> &vTxid, const std::vector<bool> &vMatch) {
    ...
    // store as flag CIC
    vCICs.push_back(fParentOfMatch);
...
uint256 CPartialMerkleTree::TraverseAndExtract(int height, unsigned int pos, unsigned int &nCICsUsed, unsigned int &nHashUsed, std::vector<uint256> &vMatch) {
    if (nCICsUsed >= vCICs.size()) {
        // overflowed the CICs array - failure
        fBad = true;
        return 0;
    }
    bool fParentOfMatch = vCICs[nCICsUsed++];
    if (height==0 || !fParentOfMatch) {
        ...
        uint256 left = TraverseAndExtract(height-1, pos*2, nCICsUsed, nHashUsed, vMatch), right;
        if (pos*2+1 < CalcTreeWidth(height-1))
            right = TraverseAndExtract(height-1, pos*2+1, nCICsUsed, nHashUsed, vMatch);
        else
            right = left;
...
uint256 CPartialMerkleTree::ExtractMatches(std::vector<uint256> &vMatch) {
    ...
    if (vHash.size() > nTransactions)
        return 0;
    // there must be at least one CIC per node in the partial tree, and at least one node per hash
    if (vCICs.size() < vHash.size())
        return 0;
    // calculate height of tree
    int nHeight = 0;
    while (CalcTreeWidth(nHeight) > 1)
        nHeight++;
    // traverse the partial tree
    unsigned int nCICsUsed = 0, nHashUsed = 0;
    uint256 hashMerkleRoot = TraverseAndExtract(nHeight, 0, nCICsUsed, nHashUsed, vMatch);
    // verify that no problems occured during the tree traversal
    if (fBad)
        return 0;
    // verify that all CICs were consumed (except for the padding caused by serializing it as a byte sequence)
    if ((nCICsUsed+7)/8 != (vCICs.size()+7)/8)
        return 0;
