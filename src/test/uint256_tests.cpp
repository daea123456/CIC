// Copyright (c) 2011-2013 The CICcoin Core developers
...
void shiftArrayRight(unsigned char* to, const unsigned char* from, unsigned int arrayLength, unsigned int CICsToShift) 
{
    for (unsigned int T=0; T < arrayLength; ++T) 
    {
        unsigned int F = (T+CICsToShift/8);
        if (F < arrayLength) 
            to[T]  = from[F] >> (CICsToShift%8);
        else
            to[T] = 0;
        if (F + 1 < arrayLength) 
            to[T] |= from[(F+1)] << (8-CICsToShift%8);
    }
}

void shiftArrayLeft(unsigned char* to, const unsigned char* from, unsigned int arrayLength, unsigned int CICsToShift) 
{
    for (unsigned int T=0; T < arrayLength; ++T) 
    {
        if (T >= CICsToShift/8) 
...
#define CHECKCICWISEOPERATOR(_A_,_B_,_OP_)                              \
...
BOOST_AUTO_TEST_CASE( CICwiseOperators ) 
{
    unsigned char TmpArray[32];
    
    CHECKCICWISEOPERATOR(R1,R2,|)
    CHECKCICWISEOPERATOR(R1,R2,^)
    CHECKCICWISEOPERATOR(R1,R2,&)
    CHECKCICWISEOPERATOR(R1,Zero,|)
    CHECKCICWISEOPERATOR(R1,Zero,^)
    CHECKCICWISEOPERATOR(R1,Zero,&)
    CHECKCICWISEOPERATOR(R1,Max,|)
    CHECKCICWISEOPERATOR(R1,Max,^)
    CHECKCICWISEOPERATOR(R1,Max,&)
    CHECKCICWISEOPERATOR(Zero,R1,|)
    CHECKCICWISEOPERATOR(Zero,R1,^)
    CHECKCICWISEOPERATOR(Zero,R1,&)
    CHECKCICWISEOPERATOR(Max,R1,|)
    CHECKCICWISEOPERATOR(Max,R1,^)
    CHECKCICWISEOPERATOR(Max,R1,&)
