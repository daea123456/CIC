...
static void add_coin(int64_t nValue, int nAge = 6*24, bool fIsFromMe = false, int nInput=0)
{
    ...
    if (fIsFromMe)
    {
        // IsFromMe() returns (GetDeCIC() > 0), and GetDeCIC() is 0 if vin.empty(),
        // so stop vin being empty, and cache a non-zero DeCIC to fake out IsFromMe()
        wtx->vin.resize(1);
        wtx->fDeCICCached = true;
        wtx->nDeCICCached = 1;
    }
...
BOOST_AUTO_TEST_CASE(coin_selection_tests)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(nValueRet, 1 * COIN);  // we should get 1 cic in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);

        BOOST_CHECK( wallet.SelectCoinsMinConf(195 * CENT, 1, 1, vCoins, setCoinsRet, nValueRet));
        BOOST_CHECK_EQUAL(nValueRet, 2 * COIN);  // we should get 2 cic in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);
