...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} cic"
...
function StartThree {
  $CICCOIND $CICCOINDARGS $B1ARGS &
  B1PID=$!
  $CICCOIND $CICCOINDARGS $B2ARGS &
  B2PID=$!
  $CICCOIND $CICCOINDARGS $B3ARGS &
  B3PID=$!
}
