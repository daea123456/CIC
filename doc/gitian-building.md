...
*Setup instructions for a gitian build of CICcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CICcoin
...
- [Building CICcoin](#building-CICcoin)
...
- Wait a CIC while 'Select and install software' runs
...
Clone the git repositories for CICcoin and gitian,
...
In [doc/release-process.md](release-process.md) in the CICcoin repository under 'Fetch and build inputs'.
...
To build CICcoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the CICcoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/CICcoin/.git/
...
For example:
```bash
URL=https://github.com/laanwj/CICcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit CICcoin=${COMMIT} --url CICcoin=${URL} ../CICcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit CICcoin=${COMMIT} --url CICcoin=${URL} ../CICcoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/CICcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/CICcoin-build.assert
```
...
[CICcoin/gitian.sigs](https://github.com/CICcoin/gitian.sigs/) repository, or
